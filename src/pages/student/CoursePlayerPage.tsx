...
// YouTube IFrame API state — loaded once, shared across all player instances
...
function YouTubePlayer({ videoId, title, className, resumeFrom = 0, onTimeUpdate }: {
    ...
    loadYouTubeApi().then(() => {
      ...
      playerRef.current = new (window as any).YT.Player(containerRef.current, {
        ...
        events: {
          onReady: () => {
            setLoading(false);
            // Poll current time every second — this is what feeds watch-time
            // tracking back up to the lesson/course player via onTimeUpdate.
