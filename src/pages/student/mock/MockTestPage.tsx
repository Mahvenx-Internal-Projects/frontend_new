...
import {
  ...
  CheckCircle2, Award, BarChart3, Users, Eye, X, BookOpen
} from 'lucide-react';
import toast from 'react-hot-toast';
import { mockTestApi, coursesApi } from '../../../services/api';
...
export default function MockTestsListPage() {
  ...
  // Fetch org courses for the dropdown
  const { data: courses = [] } = useQuery({
    queryKey: ['courses-for-exam', user?.organizationId],
    queryFn: () => coursesApi.getAll({ orgId: user?.organizationId }).then(r => r.data?.items ?? r.data ?? []),
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <label className="label flex items-center gap-1.5">
                <BookOpen className="w-3.5 h-3.5 text-gray-400" /> Link to Course (optional)
              </label>
              ...
              <p className="text-xs text-gray-400 mt-1">Students enrolled in the selected course will see this assessment under their course.</p>
            </div>
            <div className="sm:col-span-2">
              ...
              <div className="p-5 flex-1">
                ...
                {t.courseTitle && (
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <span className="text-xs text-indigo-600 font-medium">{t.courseTitle}</span>
                  </div>
                )}
                {t.description && (
